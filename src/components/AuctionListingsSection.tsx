      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[240px_minmax(0,1fr)]">
        ...
        <aside className="lg:hidden space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {filters.slice(0, 4).map((label) => (
              <button
                key={label}
                className="text-left text-xs text-foreground/80 px-2 py-2 border border-border rounded-md bg-background shadow-sm hover:border-foreground/30 transition-colors"
                type="button"
              >
                {label}
              </button>
          ...
          >
            <CarouselContent className="-ml-4">
              {Array.from({ length: 9 }).map((_, index) => (
                <CarouselItem key={index} className="pl-4 basis-[18%] sm:basis-[20%]">
                  <div
                    className="rounded-2xl overflow-hidden bg-white shadow-[0_16px_40px_rgba(15,23,42,0.08)] border border-border group h-full scale-90 sm:scale-95"
                  >
